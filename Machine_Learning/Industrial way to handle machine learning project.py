...
HEADERS = ["CodeNumber", "ClumpThickness", "UniformityCellSize", 
           "UnifoermityCellShape","MarginalAdhesion","SingleEpithelialCellSize",
...
# SimpleImputer : replace missing with medi
...
def main():
    ...
    feature_headers = HEADERS[1:-1]  # drop CodeNumber, keep all feature
    target_header = HEADER[-1]       # CancerType (2=benign, 4=malignant)
    ...
    # 8) Predictions
    predictiotions = trained_model.predict(test_x)
    ...
    print(f"Loaded model prediction for teh first test sample:{pred_loaded[0]}")
    #################################################
    # Application starter
    #################################################
